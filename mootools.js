...
Request.HTML = new Class({
	...
	success: function(text){
		...
		if (options.update) $(options.update).empty().adopt(temp.clone().childNodes); // 3n .clone()
...
Fx.Slide = new Class({
	...
	initialize: function(element, options){
		this.addEvent('complete', function(){
			...
			if (this.open) this.wrapper.setStyle(this.layout, 'auto'); // 3n
...
Fx.Scroll = new Class({
	...
	start: function(x, y){
		...
		// 3n
		if (Browser.sf2 && this.element == document.body) this.element = window
